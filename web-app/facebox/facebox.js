...
var FaceboxCloseOutside = false;

var Facebox = Class.create({
	initialize	: function(extra_set) {
		this.settings = {
			loadingImage : '/facebox/loading.png',
			closeImage   : '/facebox/closelabel.png',
			imageTypes		: new RegExp('\.' + ['png', 'jpg', 'jpeg', 'gif'].join('|') + '$', 'i'),
			inited			: true,
			faceboxHtml	: '\
		...
		$(document.body).insert({bottom: this.settings.faceboxHtml.replace('--closeImage--', this.settings.closeImage)});
	...
	watchKeyPress : function(e) {
		...
		if (!FaceboxClickOutside)
			if (e.keyCode == 27 || !Event.element(e).descendantOf(this.facebox))
				this.close();
	},
	...
	loading	: function() {
		...
		contentWrapper.insert({bottom: '<div class="loading"><img src="' + this.settings.loadingImage + '"/></div>'});
	...
	click_handler	: function(elem, e) {
		...
		//inwid
		//alert("aaaaa: " + elem.title + " , " + elem.rev);
		FaceboxClickOutside = true
		if (elem.rev != '') {
			FaceboxClickOutside = false
		}
		...
		} else if (elem.href.match(this.settings.imageTypes)) {
			var image = new Image();
			fb = this;
